...
<script>
import { getInfo, createInfo, updateInfo } from '@/api/h5/groupTags'
import { deepCopy } from '@/utils'

const form = {
  // id: undefined,
  name: undefined,
  icon: 'fas fa-user', // 设置默认图标
  color: 'rgba(64, 158, 255, 0.8)' // 设置默认颜色
}
  ...
  methods: {
    ...
    async open(type, id) {
      ...
      if (id && type === 'update') {
        ...
        // 确保新增时使用默认值
        this.formData.icon = 'fas fa-user'
    ...
    async handleRequest() {
      ...
      try {
        ...
        if (this.formType === 'create') {
          result = await createInfo(this.formData)
        } else {
          result = await updateInfo(this.formData)
        }
