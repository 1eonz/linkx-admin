...
<script>
import {  bindLabelUser, labelDetail } from '@/api/h5/quick'
import { deepCopy } from '@/utils'
import orgnaition from './orgnaition.vue'
import orgnaitionNew from './orgnaitionNew.vue'
import policerSelect from './policerSelect.vue'
...
const form = {
  ...
  orgnaztion: [],
  // 按组织分组的警员选择数据格式
  // 格式: { [orgId]: { orgName: string, path: 'parentIdLevel1,parentIdLevel2,parentIdLevel3', userIds: Array } }
  orgUsers: {}
}

export default {
  name: 'EditFrom',
  components: { policerSelect, orgnaition, orgnaitionNew },
  props: {},
  data() {
    return {
      dialogVisible: false,
  ...
  computed: {
    ...
    currentOrgUsers: {
      get() {
        ...
        // 如果当前组织有警员数据，返回警员列表
        if (this.formData.orgUsers[this.activeOrgId]) {
          return (
            this.formData.orgUsers[this.activeOrgId].userIds ||
            []
          )
        }
        // 如果当前组织没有警员数据，返回空数组
    ...
    // 已选择的组织ID列表
    selectedOrgIds() {
      return Object.keys(this.formData.orgUsers || {}).map(id =>
        Number(id)
      )
    }
  },
  ...
  methods: {
    ...
    // 处理警员选择变化
    handleUserChange(selectedItems) {
      if (!this.activeOrgId) return

      // 更新当前组织的警员选择
      if (!this.formData.orgUsers[this.activeOrgId]) {
        this.$set(this.formData.orgUsers, this.activeOrgId, {
      ...
      this.$set(
        this.formData.orgUsers[this.activeOrgId],
        'userIds',
        selectedItems || []
      )
    ...
    handleOrgSelectionChange(selectedOrgIds) {
      ...
      selectedOrgIds.forEach(orgId => {
        // 如果该组织已存在，保留原有数据
        if (this.formData.orgUsers[orgId]) {
          newOrgUsers[orgId] = this.formData.orgUsers[orgId]
        } else {
          // 新选择的组织，初始化数据结构，确保users为空数组而不是undefined
          newOrgUsers[orgId] = {
      ...
      // 更新 formData.orgUsers
      this.formData.orgUsers = newOrgUsers
    ...
    ensureOrgNames() {
      if (this.formData.orgUsers) {
        Object.keys(this.formData.orgUsers).forEach(orgId => {
          const orgData = this.formData.orgUsers[orgId]
          if (!orgData.orgName) {
          ...
          // 确保 userIds 字段存在且为数组
          if (!Array.isArray(orgData.userIds)) {
            orgData.userIds = []
          }
        })
      }
    ...
    async open(type, data, title) {
      ...
      if (data) {
        ...
        if (type === 'create') {
          ...
          // 如果获取到了 orgList，需要过滤 orgUsers
          let filteredOrgUsers = res.orgUsers || {}
          if (this.DEPARTMENT_SYNC_SIGN && this.orgList && this.orgList.length > 0) {
            ...
            // 过滤 orgUsers，将不存在的项保存到 unIncludesOrg
            filteredOrgUsers = {}
            Object.keys(res.orgUsers || {}).forEach(orgId => {
              const orgIdNum = Number(orgId)
              if (orgIdsSet.has(orgIdNum)) {
                // 组织 ID 存在于 orgList 中，保留
                filteredOrgUsers[orgId] = res.orgUsers[orgId]
              } else {
                // 组织 ID 不存在于 orgList 中，保存到 unIncludesOrg
                this.unIncludesOrg[orgId] = res.orgUsers[orgId]
              }
            })
          }
    ...
    async handleRequest() {
      ...
      try {
        ...
        // 在保存时，将 unIncludesOrg 中的项合并到 orgUsers 中
        ...
        if (this.formType === 'create') {
          const { code, msg } = await bindLabelUser(param)
          this.$message({
    ...
    validateUser(rule, value, callback) {
      ...
      if (hasSelectedOrgs) {
        // 如果有选中的部门，检查每个部门是否都有警员
        const orgUsers = this.formData.orgUsers || {}
        const selectedOrgIds = this.selectedOrgIds

        // 检查每个选中的部门是否都有警员
        const missingUsers = selectedOrgIds.filter(orgId => {
          const orgData = orgUsers[orgId]
          return (
            !orgData ||
            !orgData.userIds ||
            orgData.userIds.length === 0
          )
        })

        if (missingUsers.length > 0) {
          // 获取没有警员的部门名称
          const missingOrgNames = missingUsers.map(orgId => {
            const orgData = orgUsers[orgId]
            return orgData ? orgData.orgName : `部门${orgId}`
          })

          if (missingOrgNames.length === 1) {
            callback(new Error(`请为${missingOrgNames[0]}选择警员`))
          } else {
            callback(
              new Error(`请为以下部门选择警员：${missingOrgNames.join('、')}`)
            )
          }
        } else {
          callback()
    ...
    updateUserValidation() {
      ...
      this.$nextTick(() => {
        if (this.$refs.formRef) {
          this.$refs.formRef.validateField('userIds')
        }
      })
    },
