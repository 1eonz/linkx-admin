  ...
  <div>
    ...
    >
      <el-form
        ...
        class="dialog-form"
      >
        <el-form-item :label="$t('index.list.seatName')" prop="name">
          <el-input v-model.trim="form.name" class="edit-input" />
        </el-form-item>
        <el-form-item :label="$t('index.list.seatNumber')" prop="code">
        ...
        <el-form-item
          :label="
            `${$t('index.list.longitudeAndLatitude')}(${$t(
              'index.list.separatedByCommas'
            )})`
          "
          prop="location"
        >
          <el-input v-model="form.location" class="edit-input" />
        </el-form-item>
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button
          v-if="dialogStatus === $t('index.operations.newSeats')"
          type="primary"
...
export default {
  ...
  data() {
    ...
    const validatePass = (rule, value, callback) => {
      if (value) {
        ...
        const regLon = /^(\-|\+)?(((\d|[1-9]\d|1[0-7]\d|0{1,3})\.\d{0,6})|(\d|[1-9]\d|1[0-7]\d|0{1,3})|180\.0{0,6}|180)$/
        const regLat = /^(\-|\+)?([0-8]?\d{1}\.\d{0,6}|90\.0{0,6}|[0-8]?\d{1}|90)$/
        if (lonlat.length !== 2) {
          callback(new Error(this.$t('index.statusTitle.formatError')))
        } else if (!regLon.test(lonlat[0])) {
          callback(
            new Error(
    ...
    const rules = {
      name: [
        {
          required: true,
          message: this.$t('index.messageText.seatsNameCannotBeEmpty'),
          trigger: 'blur'
        }
      ],
      code: [
        {
          required: true,
          message: this.$t('index.messageText.seatIdCannotBeEmpty'),
          trigger: 'blur'
        },
  ...
  methods: {
    add() {
      this.dialogStatus = this.$t('index.operations.newSeats')
      this.visible = true
      this.getExtendInfo()
    ...
    modify({ id }) {
      this.dialogStatus = this.$t('index.operations.seatEditing')
