    ...
    <el-card shadow="always" class="card">
      <div class="filter-container">
        <el-input
          ...
          :placeholder="$t('index.list.pdtName')"
          style="width: 200px"
          class="filter-item"
        ...
        <el-input
          ...
          :placeholder="$t('index.list.pdtSerial')"
          style="width: 200px"
          class="filter-item"
          @keyup.enter.native="handleFilter"
        />
        ...
        <select-tree
          ...
          style="width: 200px"
          :data="deviceOptions"
          :placeholder="$t('index.list.pdtType')"
...
import pdtDetails from './components/pdtDetails'
import pdtEdit from './components/pdtEdit'
import { treeDataTranslate } from '@/utils'
import selectTree from '@/components/SelectTree'

export default {
  name: 'Device',
  components: { pagination, pdtDetails, pdtEdit, selectTree },
  filters: {
    statusFilter(status) {
      const statusMap = {
        0: 'success',
  ...
  methods: {
    ...
    getCapabilityList() {
      getEquipmentCapabilityList(this.abilityDictType).then(({ data }) => {
        this.abilityOptions = data.filter(item => {
          return (
            item.code == '512008' ||
            item.code == '512005' ||
            item.code == '512001'
          ) // 筛选，PDT装备能力只支持 GIS能力、动向回放、语音点呼
        })
      })
    },
