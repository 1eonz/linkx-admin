...
/**
 * UnattendedEditForm - 无人值守预警关系编辑表单
 *
 * 功能特性：
 * 1. 协同岗和所属组织 disabled 不可编辑
 * 2. 通知对象类型支持用户/群组两种切换
 ...
 * ```vue
 * <UnattendedEditForm :initial-data="row" @ok="handleSubmit" @cancel="handleCancel" />
 * ```
 *
 * Props:
...
defineOptions({ name: 'UnattendedEditForm' });
...
const form = reactive<UnattendedEditForm>({
  cooperId: String(initData.businessId ?? ''),
  cooperName: String(initData.businessName ?? ''),
  belongOrgId: String(initData.orgName ?? ''),
  belongOrgName: String(initData.orgName ?? ''),
...
const rules: FormRules<UnattendedEditForm> = {
...
<template>
  <div class="unattended-edit-form">
    <el-form ref="formRef" :model="form" :rules="rules" label-width="120px" style="width: 500px" size="large">
      <el-form-item label="协同岗名称" prop="cooperName">
        <el-input v-model="form.cooperName" disabled />
      </el-form-item>
